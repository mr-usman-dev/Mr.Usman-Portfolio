(function () {
  ...
  function queueSmoothScrollSync(delay = 90) {
    window.clearTimeout(smoothScrollSyncTimer);
    smoothScrollSyncTimer = window.setTimeout(() => {
      smoothScrollSyncTimer = 0;
      window.requestAnimationFrame(syncSmoothScroll);
    }, delay);
  }
  ...
  if (window.Lenis && !reducedMotionQuery.matches && !hasTouchInput) {
    ...
    modalElements.forEach((modal) => {
      ...
      modal.addEventListener('hidden.bs.modal', () => {
        ...
        queueSmoothScrollSync(0);
      });
    });
    window.addEventListener('load', () => queueSmoothScrollSync(0), { once: true });
  }
  ...
  window.addEventListener('resize', () => queueSmoothScrollSync(), { passive: true });
  window.addEventListener('orientationchange', () => queueSmoothScrollSync(120), { passive: true });
  if (window.visualViewport) {
    window.visualViewport.addEventListener('resize', () => queueSmoothScrollSync(120), { passive: true });
  }
  const navSectionLinks = document.querySelectorAll('.site-header .nav-link[href^="#"]');
  ...
  const smoothLinks = document.querySelectorAll('.smooth-link, .site-header .nav-link[href^="#"]');
  ...
  if (certificateCards.length && certificateModalElement && certificateModalImage && window.bootstrap) {
    ...
    let certificateDragFrame = 0;
    function queueCertificateDragRender() {
      if (certificateDragFrame) return;
      certificateDragFrame = window.requestAnimationFrame(() => {
        certificateDragFrame = 0;
    ...
    function finishCertificateDrag(event) {
      ...
      if (certificateDragFrame) {
        window.cancelAnimationFrame(certificateDragFrame);
        certificateDragFrame = 0;
      }
  ...
  const portfolioLightboxElement = document.getElementById('portfolioImageViewer');
  const portfolioLightboxImage = document.getElementById('portfolioLightboxImage');
  const portfolioLightboxFrame = portfolioLightboxElement?.querySelector('.portfolio-lightbox-frame');
  if (portfolioModalElement && modalImage && modalTitle) {
    ...
    let portfolioAnimationFrame = 0;
    let portfolioAnimationCommitFrame = 0;
    const portfolioImageCache = new Set();
    let portfolioLightboxFocusTarget = null;
    const portfolioLightboxState = {
      scale: 1,
      translateX: 0,
      translateY: 0,
      minScale: 1,
    ...
    function isPortfolioLightboxVisible() {
      return !!portfolioLightboxElement && !portfolioLightboxElement.hidden && portfolioLightboxElement.classList.contains('is-open');
    }
    function getPortfolioLightboxViewport() {
      if (!portfolioLightboxFrame || !portfolioLightboxImage) return null;
      const frameWidth = portfolioLightboxFrame.clientWidth;
      const frameHeight = portfolioLightboxFrame.clientHeight;
      const naturalWidth = portfolioLightboxImage.naturalWidth || frameWidth;
      const naturalHeight = portfolioLightboxImage.naturalHeight || frameHeight;
      if (!frameWidth || !frameHeight || !naturalWidth || !naturalHeight) return null;
      const fitRatio = Math.min(frameWidth / naturalWidth, frameHeight / naturalHeight);
      return {
        frameWidth,
        frameHeight,
        baseWidth: naturalWidth * fitRatio,
        baseHeight: naturalHeight * fitRatio
      };
    }
    function clampPortfolioLightboxTransform(scale, translateX, translateY) {
      const clampedScale = Math.min(portfolioLightboxState.maxScale, Math.max(portfolioLightboxState.minScale, scale));
      if (clampedScale <= portfolioLightboxState.minScale + 0.001) {
        return {
          scale: 1,
          translateX: 0,
          translateY: 0
      ...
      const viewport = getPortfolioLightboxViewport();
      if (!viewport) {
        return {
          scale: clampedScale,
          translateX,
      ...
      const maxOffsetX = Math.max(0, ((viewport.baseWidth * clampedScale) - viewport.frameWidth) / 2);
      const maxOffsetY = Math.max(0, ((viewport.baseHeight * clampedScale) - viewport.frameHeight) / 2);
      return {
        scale: clampedScale,
    ...
    function renderPortfolioLightboxTransform({ immediate = false } = {}) {
      if (!portfolioLightboxImage) return;
      portfolioLightboxImage.classList.toggle('is-interacting', immediate);
      portfolioLightboxImage.classList.toggle('is-zoomed', portfolioLightboxState.scale > 1.01);
      portfolioLightboxImage.style.transform = `translate3d(${portfolioLightboxState.translateX}px, ${portfolioLightboxState.translateY}px, 0) scale(${portfolioLightboxState.scale})`;
    }
    function suppressPortfolioLightboxClose(duration = 320) {
      const now = window.performance?.now?.() || Date.now();
      portfolioLightboxState.suppressCloseUntil = now + duration;
    }
    function shouldSuppressPortfolioLightboxClose() {
      const now = window.performance?.now?.() || Date.now();
      return now < portfolioLightboxState.suppressCloseUntil;
    }
    function setPortfolioLightboxTransform(scale, translateX, translateY, options = {}) {
      const nextState = clampPortfolioLightboxTransform(scale, translateX, translateY);
      portfolioLightboxState.scale = nextState.scale;
      portfolioLightboxState.translateX = nextState.translateX;
      portfolioLightboxState.translateY = nextState.translateY;
      renderPortfolioLightboxTransform(options);
    }
    function resetPortfolioLightboxTransform(options = {}) {
      portfolioLightboxState.pinchDistance = 0;
      portfolioLightboxState.pinchScale = 1;
      setPortfolioLightboxTransform(1, 0, 0, options);
    }
    function zoomPortfolioLightbox(nextScale, clientX, clientY, options = {}) {
      const nextClampedScale = Math.min(portfolioLightboxState.maxScale, Math.max(portfolioLightboxState.minScale, nextScale));
      const currentScale = portfolioLightboxState.scale || 1;
      if (!portfolioLightboxFrame || nextClampedScale === currentScale) {
        setPortfolioLightboxTransform(nextClampedScale, portfolioLightboxState.translateX, portfolioLightboxState.translateY, options);
        return;
      }
      const frameRect = portfolioLightboxFrame.getBoundingClientRect();
      const pointX = typeof clientX === 'number' ? clientX - frameRect.left - (frameRect.width / 2) : 0;
      const pointY = typeof clientY === 'number' ? clientY - frameRect.top - (frameRect.height / 2) : 0;
      const scaleRatio = nextClampedScale / currentScale;
      const nextTranslateX = pointX - ((pointX - portfolioLightboxState.translateX) * scaleRatio);
      const nextTranslateY = pointY - ((pointY - portfolioLightboxState.translateY) * scaleRatio);
      setPortfolioLightboxTransform(nextClampedScale, nextTranslateX, nextTranslateY, options);
    }
    function syncPortfolioLightboxPinchState() {
      const activePointers = Array.from(portfolioLightboxState.pointers.values());
      if (activePointers.length < 2) {
        portfolioLightboxState.pinchDistance = 0;
        portfolioLightboxState.pinchScale = portfolioLightboxState.scale;
        return null;
      }
      const [firstPointer, secondPointer] = activePointers;
      const distance = Math.hypot(secondPointer.x - firstPointer.x, secondPointer.y - firstPointer.y) || 1;
    ...
    function beginPortfolioLightboxDrag(clientX, clientY) {
      portfolioLightboxState.dragStartClientX = clientX;
      portfolioLightboxState.dragStartClientY = clientY;
      portfolioLightboxState.dragOriginX = portfolioLightboxState.translateX;
      portfolioLightboxState.dragOriginY = portfolioLightboxState.translateY;
    }
    function isPortfolioLightboxPointInsideImage(clientX, clientY) {
      if (!portfolioLightboxImage) return false;
      const imageRect = portfolioLightboxImage.getBoundingClientRect();
      return clientX >= imageRect.left &&
        clientX <= imageRect.right &&
        clientY >= imageRect.top &&
        clientY <= imageRect.bottom;
    }
    function handlePortfolioLightboxTapPoint(clientX, clientY) {
      if (typeof clientX !== 'number' || typeof clientY !== 'number') return;
      const now = window.performance?.now?.() || Date.now();
      const tapDistance = Math.hypot(clientX - portfolioLightboxState.lastTapX, clientY - portfolioLightboxState.lastTapY);
      if ((now - portfolioLightboxState.lastTapTime) < 280 && tapDistance < 28) {
        const nextScale = portfolioLightboxState.scale > 1.6 ? 1 : 2.2;
        zoomPortfolioLightbox(nextScale, clientX, clientY, { immediate: false });
        suppressPortfolioLightboxClose();
        portfolioLightboxState.lastTapTime = 0;
        return;
      }
      portfolioLightboxState.lastTapTime = now;
      portfolioLightboxState.lastTapX = clientX;
      portfolioLightboxState.lastTapY = clientY;
    }
    function preventPortfolioLightboxBrowserZoom(event) {
      if (!isPortfolioLightboxVisible() || !event.cancelable) return;
      event.preventDefault();
    }
    ...
    function preloadImage(source) {
      const normalizedSource = normalizePortfolioImagePath(source);
      if (!normalizedSource || portfolioImageCache.has(normalizedSource)) {
        return Promise.resolve();
      }
      portfolioImageCache.add(normalizedSource);
    ...
    function preloadPortfolioNeighbors() {
      if (portfolioModalState.slides.length < 2) return;
      const nextIndex = (portfolioModalState.index + 1) % portfolioModalState.slides.length;
      const previousIndex = (portfolioModalState.index - 1 + portfolioModalState.slides.length) % portfolioModalState.slides.length;
      preloadImage(portfolioModalState.slides[nextIndex]?.image);
      preloadImage(portfolioModalState.slides[previousIndex]?.image);
    }
    function clearPortfolioAnimationState() {
      ...
      if (portfolioAnimationFrame) {
        window.cancelAnimationFrame(portfolioAnimationFrame);
        portfolioAnimationFrame = 0;
      }
      if (portfolioAnimationCommitFrame) {
        window.cancelAnimationFrame(portfolioAnimationCommitFrame);
        portfolioAnimationCommitFrame = 0;
      }
      window.clearTimeout(portfolioAnimationTimer);
    }
    function queuePortfolioAnimation() {
      clearPortfolioAnimationState();
      portfolioAnimationFrame = window.requestAnimationFrame(() => {
        portfolioAnimationFrame = 0;
        portfolioAnimationCommitFrame = window.requestAnimationFrame(() => {
          portfolioAnimationCommitFrame = 0;
          if (portfolioModalStage) portfolioModalStage.classList.add('is-animating');
          if (portfolioModalInfo) portfolioModalInfo.classList.add('is-animating');
    ...
    function closePortfolioLightbox() {
      if (!portfolioLightboxElement || portfolioLightboxElement.hidden) return;
      portfolioLightboxElement.classList.remove('is-open');
      portfolioLightboxElement.hidden = true;
      portfolioLightboxElement.setAttribute('aria-hidden', 'true');
      document.body.classList.remove('portfolio-lightbox-open');
      portfolioLightboxState.pointers.clear();
      portfolioLightboxState.lastTapTime = 0;
      resetPortfolioLightboxTransform({ immediate: false });
      if (portfolioLightboxImage) {
        portfolioLightboxImage.src = '';
        portfolioLightboxImage.alt = 'Portfolio full screen preview';
      }
      if (portfolioLightboxFocusTarget instanceof HTMLElement && portfolioLightboxFocusTarget.isConnected) {
        portfolioLightboxFocusTarget.focus({ preventScroll: true });
      }
      portfolioLightboxFocusTarget = null;
    }
    function openPortfolioLightbox() {
      const slide = portfolioModalState.slides[portfolioModalState.index];
      if (!portfolioLightboxElement || !portfolioLightboxImage || !slide?.image) return;
      portfolioLightboxFocusTarget = document.activeElement instanceof HTMLElement ? document.activeElement : null;
      portfolioLightboxImage.src = slide.image;
      portfolioLightboxImage.alt = modalImage.alt || `${portfolioModalState.projectTitle} full screen preview`;
      portfolioLightboxElement.hidden = false;
      portfolioLightboxElement.setAttribute('aria-hidden', 'false');
      portfolioLightboxElement.classList.add('is-open');
      document.body.classList.add('portfolio-lightbox-open');
      portfolioLightboxState.pointers.clear();
      resetPortfolioLightboxTransform({ immediate: false });
      portfolioLightboxElement.focus({ preventScroll: true });
    }
    ...
    function renderPortfolioSlide(options = {}) {
      ...
      syncPortfolioDots();
      preloadPortfolioNeighbors();
      if (isPortfolioLightboxVisible() && portfolioLightboxImage) {
        portfolioLightboxImage.src = slide.image;
        portfolioLightboxImage.alt = modalImage.alt;
        resetPortfolioLightboxTransform({ immediate: false });
      }
    }
    ...
    portfolioCards.forEach((card) => {
      card.addEventListener('click', () => {
        ...
        preloadImage(primarySlide?.image);
        portfolioModal.show();
      });
    });
    if (modalPrevSlide) {
    ...
    if (portfolioModalStage) {
      portfolioModalStage.addEventListener('click', (event) => {
        if (!(event.target instanceof Element)) return;
        if (event.target.closest('.portfolio-gallery-arrow')) return;
        if (event.target.closest('.portfolio-modal-counter')) return;
        event.stopPropagation();
        openPortfolioLightbox();
      });
    }
    if (portfolioLightboxElement) {
      const portfolioLightboxBody = portfolioLightboxElement.querySelector('.portfolio-lightbox-body');
      portfolioLightboxImage?.addEventListener('load', () => {
        if (!isPortfolioLightboxVisible()) return;
        resetPortfolioLightboxTransform({ immediate: false });
      });
      portfolioLightboxImage?.addEventListener('dblclick', (event) => {
        event.preventDefault();
        const nextScale = portfolioLightboxState.scale > 1.6 ? 1 : 2.35;
        zoomPortfolioLightbox(nextScale, event.clientX, event.clientY, { immediate: false });
        suppressPortfolioLightboxClose();
      });
      portfolioLightboxImage?.addEventListener('dragstart', (event) => {
        event.preventDefault();
      });
      portfolioLightboxFrame?.addEventListener('pointerdown', (event) => {
        if (!isPortfolioLightboxVisible()) return;
        if (event.pointerType === 'mouse' && event.button !== 0) return;
        if (!isPortfolioLightboxPointInsideImage(event.clientX, event.clientY) && event.pointerType === 'mouse') return;
        if (event.cancelable) event.preventDefault();
        portfolioLightboxState.pointers.set(event.pointerId, {
          x: event.clientX,
          y: event.clientY
        });
        portfolioLightboxState.moved = false;
        try {
          portfolioLightboxFrame.setPointerCapture(event.pointerId);
        } catch (_error) {
        }
        if (portfolioLightboxState.pointers.size === 1) {
          beginPortfolioLightboxDrag(event.clientX, event.clientY);
        } else {
          const pinchState = syncPortfolioLightboxPinchState();
          if (pinchState) {
            portfolioLightboxState.pinchDistance = pinchState.distance;
            portfolioLightboxState.pinchScale = portfolioLightboxState.scale;
          }
        }
      });
      portfolioLightboxFrame?.addEventListener('pointermove', (event) => {
        if (!portfolioLightboxState.pointers.has(event.pointerId) || !isPortfolioLightboxVisible()) return;
        if (event.cancelable) event.preventDefault();
        portfolioLightboxState.pointers.set(event.pointerId, {
          x: event.clientX,
          y: event.clientY
        });
        if (portfolioLightboxState.pointers.size >= 2) {
          const pinchState = syncPortfolioLightboxPinchState();
          if (!pinchState) return;
          if (!portfolioLightboxState.pinchDistance) {
            portfolioLightboxState.pinchDistance = pinchState.distance;
            portfolioLightboxState.pinchScale = portfolioLightboxState.scale;
          }
          portfolioLightboxState.moved = true;
          suppressPortfolioLightboxClose();
          const pinchScale = portfolioLightboxState.pinchScale * (pinchState.distance / portfolioLightboxState.pinchDistance);
          zoomPortfolioLightbox(pinchScale, pinchState.midpointX, pinchState.midpointY, { immediate: true });
          return;
        }
        if (portfolioLightboxState.scale <= 1.01) return;
        const nextTranslateX = portfolioLightboxState.dragOriginX + (event.clientX - portfolioLightboxState.dragStartClientX);
        const nextTranslateY = portfolioLightboxState.dragOriginY + (event.clientY - portfolioLightboxState.dragStartClientY);
        if (Math.hypot(event.clientX - portfolioLightboxState.dragStartClientX, event.clientY - portfolioLightboxState.dragStartClientY) > 4) {
          portfolioLightboxState.moved = true;
          suppressPortfolioLightboxClose();
        }
        setPortfolioLightboxTransform(portfolioLightboxState.scale, nextTranslateX, nextTranslateY, { immediate: true });
      });
      const finishPortfolioLightboxPointer = (event) => {
        if (!portfolioLightboxState.pointers.has(event.pointerId)) return;
        if (event.cancelable) event.preventDefault();
        portfolioLightboxState.pointers.delete(event.pointerId);
        try {
          portfolioLightboxFrame?.releasePointerCapture(event.pointerId);
        } catch (_error) {
        }
        if (portfolioLightboxState.pointers.size >= 2) {
          const pinchState = syncPortfolioLightboxPinchState();
        ...
        if (portfolioLightboxState.pointers.size === 1) {
          const remainingPointer = Array.from(portfolioLightboxState.pointers.values())[0];
          beginPortfolioLightboxDrag(remainingPointer.x, remainingPointer.y);
          portfolioLightboxState.pinchDistance = 0;
          portfolioLightboxState.pinchScale = portfolioLightboxState.scale;
          return;
        }
        portfolioLightboxState.pinchDistance = 0;
        portfolioLightboxState.pinchScale = portfolioLightboxState.scale;
        renderPortfolioLightboxTransform({ immediate: false });
        if (!portfolioLightboxState.moved) {
          if (isPortfolioLightboxPointInsideImage(event.clientX, event.clientY)) {
            if (event.pointerType !== 'mouse') {
              handlePortfolioLightboxTapPoint(event.clientX, event.clientY);
            }
          } else if (!shouldSuppressPortfolioLightboxClose()) {
            closePortfolioLightbox();
          }
        }
      };
      portfolioLightboxFrame?.addEventListener('pointerup', finishPortfolioLightboxPointer);
      portfolioLightboxFrame?.addEventListener('pointercancel', finishPortfolioLightboxPointer);
      portfolioLightboxElement.addEventListener('wheel', (event) => {
        if (!isPortfolioLightboxVisible()) return;
        if (!(event.target instanceof Element) || !event.target.closest('.portfolio-lightbox-dialog')) return;
        if (event.cancelable) event.preventDefault();
        const zoomSensitivity = event.ctrlKey ? 0.0065 : 0.0024;
        const zoomFactor = Math.exp(-event.deltaY * zoomSensitivity);
        zoomPortfolioLightbox(portfolioLightboxState.scale * zoomFactor, event.clientX, event.clientY, { immediate: false });
        suppressPortfolioLightboxClose();
      }, { passive: false });
      ['gesturestart', 'gesturechange', 'gestureend'].forEach((eventName) => {
        portfolioLightboxElement.addEventListener(eventName, preventPortfolioLightboxBrowserZoom, { passive: false });
        document.addEventListener(eventName, preventPortfolioLightboxBrowserZoom, { passive: false });
      });
      portfolioLightboxBody?.addEventListener('click', (event) => {
        if (shouldSuppressPortfolioLightboxClose()) return;
        if (isPortfolioLightboxPointInsideImage(event.clientX, event.clientY)) return;
        closePortfolioLightbox();
      });
      portfolioLightboxElement.addEventListener('click', (event) => {
        if (!(event.target instanceof Element)) return;
        if (event.target.closest('.portfolio-lightbox-dialog')) return;
        closePortfolioLightbox();
      });
      window.addEventListener('resize', () => {
        if (!isPortfolioLightboxVisible()) return;
        setPortfolioLightboxTransform(portfolioLightboxState.scale, portfolioLightboxState.translateX, portfolioLightboxState.translateY, { immediate: false });
      });
    }
    document.addEventListener('keydown', (event) => {
      if (isPortfolioLightboxVisible()) {
        if (event.key === 'ArrowLeft') {
          event.preventDefault();
          stepPortfolioSlide(-1);
          return;
